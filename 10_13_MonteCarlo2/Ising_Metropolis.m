...
for jj = 1:500
    ...
    for k = 1 : 10000
        ...
        neighborhood( find( [iy == 1    iy == pixelY    ix == 1   ix == pixelX] ) ) = []; % Prevent exceeding the border
    
        % pesky boundaries...thrash them
        nagree =  sum( Fc == F(neighborhood) ); % Jolin:new state
        ndisagree = sum( Fc ~= F(neighborhood) ); % Jolin:old state
